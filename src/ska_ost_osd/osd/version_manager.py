...
def manage_version_release(cycle_id: str, release_type: Optional[str] = None) -> Tuple[str, str]:
    """Manage version release for a given cycle ID.
    
    Args:
        cycle_id (str): The cycle ID for version mapping
        release_type (Optional[str]): Type of release ('major' or 'minor', defaults to patch)
    ...
    version_mapping_path = project_root / "tmdata/version_mapping/cycle_gitlab_release_version_mapping.json"
    ...
    with open(version_mapping_path, 'w') as f:
    ...
    with open(latest_release_path, 'w') as f:
