...
from ska_osd.osd.osd import osd_tmdata_source, get_osd_data
...
def get_osd(**kwargs):
    ...
    tm_data_sources = osd_tmdata_source(cycle_id=cycle_id, osd_version=osd_version, source=source, gitlab_branch=gitlab_branch)
    tm_data = TMData(source_uris=tm_data_sources, update=True)
    tmdata_1_gitlab_data = get_osd_data(capabilities=[capabilities], tmdata=tm_data, array_assembly=array_assembly)
