...
from ska_ost_osd.rest.api.resources import release_osd_data


class TestResources:
    @patch("ska_ost_osd.rest.api.resources.manage_version_release")
    @patch("ska_ost_osd.rest.api.resources.push_to_gitlab")
    def test_release_osd_data_2(self, mock_push_to_gitlab, mock_manage_version_release):
        """
        Test release_osd_data with valid cycle_id and invalid release_type.
        """
        # Arrange
        cycle_id = 1
    ...
    @patch("ska_ost_osd.rest.api.resources.manage_version_release")
    @patch("ska_ost_osd.rest.api.resources.push_to_gitlab")
    @patch("ska_ost_osd.rest.api.resources.PUSH_TO_GITLAB", "1")
    def test_release_osd_data_success(
        self, mock_push_to_gitlab, mock_manage_version_release
    ):
        """
        Test successful release of OSD data with valid cycle_id and no release_type.
        """
        # Arrange
        mock_manage_version_release.return_value = ("1.0.1", "cycle_1")
