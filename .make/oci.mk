...
CAR_OCI_USE_HARBOR ?=## Set to true to use Harbor instead of Nexus
...
$(info Setting CAR_OCI_REGISTRY_HOST to `$(CAR_OCI_REGISTRY_HOST)`)
endif
...
OCI_IMAGE_DIRS := $(shell if [ -d images ]; then cd images > /dev/null; for name in $$(ls); do if [ -d $$name ]; then echo $$name; fi done else echo $(PROJECT_NAME);  fi;)
...
oci-post-publish:
#	notation sign $(CAR_OCI_REGISTRY_HOST)/$(OCI_IMAGE):$(VERSION); \ # TODO: Uncomment once we officially migrate to Harbor
