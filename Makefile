...
K8S_CHARTS ?= ska-oso-osd $(K8S_CHART)
HELM_CHART ?= ska-oso-osd-umbrella
...
K8S_CHART_PARAMS += --set ska-oso-osd.rest.image.tag=$(VERSION)-dev.c$(CI_COMMIT_SHORT_SHA) \
	--set ska-oso-osd.rest.image.registry=$(CI_REGISTRY)/ska-telescope/db/ska-oso-osd
endif
...
rest: ## start OSD REST server
	docker run --rm -p 5000:5000 --name=$(PROJECT_NAME) $(IMAGE_TO_TEST) gunicorn --chdir src \
		--bind 0.0.0.0:5000 --logger-class=ska_oso_osd.rest.wsgi.UniformLogger --log-level=$(LOG_LEVEL) ska_oso_osd.rest.wsgi:app
...
dev-up: K8S_CHART_PARAMS += --set ska-oso-osd.rest.image.tag=$(VERSION) --set ska-oso-osd.rest.backend.type=filesystem
...
# The docs build fails unless the ska-oso-osd package is installed locally as importlib.metadata.version requires it.
docs-pre-build:
